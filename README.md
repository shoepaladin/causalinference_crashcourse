...
### 6. Panel Models
This covers panel methods that build a treated unit's counterfactual over time, framed estimand-first around the ATT. It walks through **difference-in-differences** (the two-differences derivation, TWFE, parallel-trends and no-anticipation assumptions, the event study, and cautions on low-powered pre-trend tests and staggered-adoption "forbidden comparisons"), **synthetic control** (ADH simplex weights and the Doudchenko–Imbens elastic-net variant, with placebo/conformal inference), and **synthetic difference-in-differences** (Arkhangelsky et al. 2021), which hedges the two by weighting both units and time periods. A companion simulation notebook ([Notebooks/6 Panel Models.ipynb](Notebooks/6%20Panel%20Models.ipynb)) uses the `panelib` library from the [statanomics](https://github.com/shoepaladin/statanomics) repo to compare DiD, SC-ADH, SC-DI, and SDID on simulated panels under parallel trends and a parallel-trends violation, reporting bias and confidence-interval coverage with a shared conformal inference procedure.
...
This covers conformal inference as distribution-free, finite-sample uncertainty quantification, then specializes it to causal panels. It teaches the general method the way it is taught in the literature (split conformal, the exchangeability assumption, the marginal coverage guarantee, and the marginal-vs-conditional limitation, following Vovk et al., Lei et al., and Angelopoulos–Bates), covers what breaks exchangeability in time series and the two fixes (moving-block permutations and weighted/nonexchangeable conformal, per Chernozhukov–Wüthrich–Zhu and Barber et al.), and finishes with the causal payoff: exact, robust inference for **synthetic controls** with a single treated unit ([Chernozhukov, Wüthrich, Zhu 2021](https://www.tandfonline.com/doi/full/10.1080/01621459.2021.1920957)). Figures are generated from a real synthetic-control fit using the `conformal_inf` API in the [statanomics](https://github.com/shoepaladin/statanomics) `panelib` library.
